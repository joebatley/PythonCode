...
### IMPORT FILE ###
sample = 'SC020'
datadir = '/Volumes/data/Projects/Spincurrents/Joe Batley/Measurements/SC020/Transport/Scattering Analysis/'

R = workfile(datadir+'SC020_1_B_Cu_resistivity_vs_T.txt')       #Resistivity of LSV spacer
#R.rename(r'$\rho$ ($\Omega$m)','res')

L = workfile(datadir+'SC020_Spindiffusion_length_vs_T.txt')     #Spin diffusion length data
...
res_params.add('K', value= 1.309e-7,min=1e-8,max=1e-6,vary=True)
res_params.add('Dt', value= 288,min=250,max=400,vary=True)
res_params.add('j', value= 7e-12,min=7e-12,max=5e-6,vary=True)
...
sc_params.add('e_m', value=.3e-2 ,min=3e-5,vary=True)
...
tau.plot_xy('T (K)','tsf.plot', yerr='t_err.plot',linestyle='',linewidth=3,marker='o',markersize=5,label=None)
...
tau.plot_xy('tau_e','1/tsf',linestyle='',linewidth=3,marker='o',markersize=5,label=None)
tau.ylabel = r'$\tau_{sf}$ (ps)'
