...
params.add('j', value= 7e-11,min=7e-12,max=5e-6,vary=True)
#params.add('Tk', value= 10,min=5,max=50,vary=True)

R.subtract('res',min(R.column('res')),header='res.norm')
# do fit, here with leastsq model
result = minimize(BG, params, args=(R.column('T (K)'), R.column('res.norm')))

# calculate final result
final = R.column('res.norm') + result.residual + min(R.column('res'))+1.8e-11
...
print 'rho0 = ' +str(min(R.column('res')))#+format_error(params['rho_0'].value,params['rho_0'].stderr,latex=True)
### GET SCATTERING TIME ###

rho = R.interpolate(L.column('T'),xcol='T (K)')
print R.column_headers
tsf = L.column('Lam_Cu')**2*rho[:,3]*(1.6e-19)*1.81e28
...
def phonon(sc_params,t,tau):
    ...
    K = params['K'].value
    Dt = params['Dt'].value 
    rho_0 = min(R.column('res'))+1.8e-11#params['rho_0'].value    
    j = params['j'].value
    #Tk = params['Tk'].value 
...
sc_params.add('m', value=.3e-3 ,min=4e-1,vary=True)
...
BG.plot_xy('T (K)','res',linestyle='',linewidth=3,marker='o',markersize=5,label=None)#,label = r'Cu spacer $\rho$')
BG.multiply('BG',1e8,replace=True,header='BG')
BG.plot_xy('T (K)','BG',linestyle='-',linewidth=2,marker='',label=None)#,label = 'B-G fit')
...
### Differentiate ###
print BG.column_headers
grad = workfile(BG)
#grad.sort('Time')
cols = ('T (K)','res')
diff = BG.SG_Filter(col=cols, points=15, poly=1, order=1,result=None, replace=False, header=None)
grad.add_column(diff,'BG_diff')
grad.figure()
...
grad.plot_xy('T (K)','BG_diff',linestyle='-',linewidth=2,marker='',label=None)#,label = 'B-G fit')
grad.ylabel = r'd$\rho$/dT'
plt.tight_layout()
