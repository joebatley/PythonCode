...
pattern = '*BetavsT.txt'
folder = DataFolder('/Volumes/stonerlab.leeds.ac.uk - -storage/data/Projects/Spincurrents/Joe Batley/Measurements/SC004/Transport/Beta_vs_sep',pattern = pattern,type=workfile)  
...
Sep=[]
for f in folder:
    b.append(interpolate.interp1d(f.column('T'),f.column('Beta')))
...
for t in plot_T:
    decay.plot_xy(r'L ($\mu$m)',r'$\beta$'+str(t),yerr=r'$\beta$_err'+str(t),linestyle='',marker='o',markersize=6,label=str(t)+' K')
    decay.ylabel=r'$\beta$ (V/A$^2$)'
plt.tight_layout()
plt.legend(loc='best')
plt.semilogy()
