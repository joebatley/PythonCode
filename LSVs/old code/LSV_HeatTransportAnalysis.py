...
def P_AP(folder,keys):
  ...
  for f in folder:
    ...
    #    y=(i**2)*R - (Ks*As/dz)*(x-Ts) - (Kcu*Acu/dx)*(x-Td) - As*sig*emis*(x**4-Ts**4) - Asc*sig*emis*0.2*(Td**5-x**5) - Asc*sig*emis*Ts**4
    #    return y
    
    def heat(x,i,R,Ks,As,dz,Kcu,Acu,dx,sig,emis,Asc,Td,Ts): 
        
        y = (i**2)*R - (Ks*As/dz)*(x-Ts) - (Kcu*Acu/dx)*(x-Td) - (Kcu*Acu/5e-6)*(x-Ts) #(Ks*Asc/dz)*(0.5*Td**2-Ts*Td-0.5*x**2+Ts*x)    
        #Tk = (x+((15*Ks)/(dz*x**3))*Ts)/(((15*Ks)/(dz*x**3)) + 1)-y
        return y
...
for column in folder['P'].column_headers:
  ...
  if column == '60':
    ...
    ax1.plot(int(column),folder['P'].column(column)[4],'bo')
    ax1.plot(int(column),folder['P'].column(column)[5],'ro')
    ax1.set_ylabel('T$_{interface}$-T$_{substrate}$', color='k')
